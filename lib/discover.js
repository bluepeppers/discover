  ...
  async.parallel({
    ...
    publishedContainers: function (done) {
      self.etcd.get(self.containerPrefix, function (err, result) {
        // Handle not found error as a special case
    ...
    if (err) {
      winston.error('An error occured reconciling published containers due to: ', err);
      return;
    }
    ...
    if (results.publishedContainers && results.publishedContainers.node && results.publishedContainers.node.nodes) {
    ...
    winston.debug('Running container list from Docker: ', running);
    winston.debug('Published container list from Etcd: ', published);
    ...
    try {
      services.forEach(function (service) {
        var servicePath = self.servicePrefix + service.name + '/' + service.realm + '/' + self.hostId + '/' + service.containerId,
            serviceIndexPath = self.servicePrefix + service.name + '/_index';
        ...
        self.etcd.set(servicePath, service.location, function (err) {
          ...
          self.etcd.set(serviceIndexPath, (new Date).getTime(), function(err) {
            if (err) {
              winston.warn('Unable to change service index key due to: ' + JSON.stringify(err));
            }
          });
        });
    ...
    try {
      services.forEach(function (service) {
        var servicePath = published[service],
            indexPath = self.servicePrefix + service + '/_index',
            errors = [];
        self.etcd.del(servicePath, { recursive: true }, function (err) {
          ...
          self.etcd.set(indexPath, (new Date).getTime(), function(err) {
            if (err) {
              winston.error('Unable to change service index key due to: ' + JSON.stringify(err));
              errors.push(err);
            }
          });
        });
